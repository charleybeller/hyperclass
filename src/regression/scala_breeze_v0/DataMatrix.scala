...
//		extractFeatures();
	...
	def extractFeatures() = {
		...
		for(p <- pairs ){
			var idx : Int = 1;
			var wordFeatures : ArraySeq[Int] = new ArraySeq[Int](0);
			for(f <- parseFeatures(p)){
				wordFeatures :+ 0; //(new Int(idx, f));
				idx = idx + 1;
			}
 	...
 	*/
	def getX() : ArraySeq[Array[Int]] = {
		
		var X : ArraySeq[Array[Int]] = new ArraySeq[Array[Int]](0);
		return X;
//		for(var p <- pairs ){
//			copyToArray( X :+ (p.features.toArray(new Int[ p.features.size() ]));
//		}
//		return X.toArray(new Int[ X.size() ][ X.get(0).length ]);
